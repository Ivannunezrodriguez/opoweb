...
const release = window.OPOWEB_CARRANQUE_TEORIA_V85_BLOQUE4;
assert.ok(ope, 'No se encontró Carranque');
assert.ok(release, 'No se cargó el bloque 16-20');
assert.deepStrictEqual(plain(release.themes), [16,17,18,19,20]);
assert.deepStrictEqual(plain(release.completedThemes), Array.from({ length: 20 }, (_, index) => index + 1));
assert.deepStrictEqual(plain(release.pendingThemes), []);
assert.equal(release.autonomous, true);
assert.ok(Array.isArray(release.dynamicChecks) && release.dynamicChecks.length === 3);
...
for (const number of release.themes) {
  ...
  assert.ok(theme.sections.length >= 15, `Tema ${number}: pocas secciones (${theme.sections.length})`);
  ...
  assert.ok(theme.officialSources.every(source => /^BOE-A-/.test(source.reference)), `Tema ${number}: fuente no oficial`);
  ...
  assert.ok(Array.isArray(theme.studyWarnings) && theme.studyWarnings.length === 2);
...
assert.ok(total >= 8000, `Bloque 16-20 demasiado corto: ${total}`);

const allAutonomous = ope.themes.every(theme => {
  const text = `${(theme.sections || []).flatMap(section => [section.heading, ...(section.paragraphs || [])]).join(' ')} ${theme.tree || ''} ${(theme.reviewTable || []).flat().join(' ')}`;
  return theme.theoryStatus?.autonomous === true && wordCount(text) >= 1000 && (theme.officialSources || []).length >= 2;
});
assert.equal(allAutonomous, true, 'Carranque no tiene todavía los 20 temas autosuficientes');

fs.writeFileSync('carranque-teoria-v85-bloque4.json', JSON.stringify({
  ...
  dynamicChecks: release.dynamicChecks,
  carranqueAutonomous: allAutonomous
}, null, 2));

console.log(`Carranque teoría v0.85 bloque 4 APTO · ${total} palabras · mínimo por tema ${minimum} · 20/20 autosuficientes`);
