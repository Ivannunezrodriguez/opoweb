...
assert.equal(packageJson.version, '0.89.1');
...
assert.ok(ui89.includes("const CACHE = 'opoweb-v95'"));
assert.ok(ui89.includes("'#theoryStatusV85, #pueblaTheoryStatusV86, #programmeStatusV89'"));
assert.ok(ui89.includes('window.OPOWEB_RELEASE_V89'));
assert.ok(ui89.includes('Reserva OAPGT · tema 22'));
...
assert.ok(browserTest.includes('Versión OpoWeb v0.89.1'));
assert.ok(browserTest.includes("includes('opoweb-v95')"));
assert.ok(browserTest.includes('/assets/js/ui-v89-release.js'));
assert.ok(browserTest.includes('OPOWEB_RELEASE_V89'));

console.log(`Integridad v0.89.1 OK · ${manifest.scripts.length} módulos · ${manifest.allAssets.length} recursos · interfaz unificada`);