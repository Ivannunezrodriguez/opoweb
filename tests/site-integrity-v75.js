...
assert.ok(/const CACHE = 'opoweb-v82';/.test(serviceWorker), 'La caché no es opoweb-v82');
...
assert.ok(index.includes('assets/js/uc3m-v75-estatutos.js'), 'Falta cargar UC3M v0.75');
assert.ok(index.includes('assets/js/auditoria-calidad-v72.js'), 'Falta cargar auditoría de calidad');
assert.ok(index.includes('assets/js/ui-v75.js'), 'Falta cargar interfaz v0.75');

console.log(`Integridad web v0.75 OK · ${indexAssets.length} referencias · ${cachedAssets.length} recursos PWA`);
