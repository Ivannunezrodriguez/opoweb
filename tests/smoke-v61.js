...
for (const version of [56, 57, 58, 59, 60, 61]) {
...
const counts = { 17: 30, 18: 34, 19: 33, 20: 45, 21: 51, 23: 37 };
...
const theme23 = ope.themes.find(item => item.number === 23);
assert.equal(ope.globalTestAudit.targetThemes, 22);
assert.equal(window.OPOWEB_DIPUTACION_V61.addedQuestions, 22);
assert.equal(window.OPOWEB_DIPUTACION_V61.cumulativeManualQuestions, 965);
assert.equal(window.OPOWEB_DIPUTACION_V61.cumulativeSecondPassQuestions, 380);
assert.ok(theme23.sections.some(section => section.heading === 'Consulta y participación'));
