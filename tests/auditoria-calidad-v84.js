...
report.version = 'v0.84.0';
report.date = '2026-07-13';
if (report.summary) report.summary.version = 'v0.84.0';
report.municipal = municipal;
fs.writeFileSync('audit-v84.json', JSON.stringify(report, null, 2));

assert.equal(report.oposiciones.length, 4);
assert.equal(report.totalQuestions, 3102, `Total inesperado: ${report.totalQuestions}`);
...
assert.equal(totals['diputacion-toledo-admin-2026'], 1242);
