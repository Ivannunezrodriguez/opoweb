...
report.version = 'v0.73.0';
report.date = '2026-07-12';
if (report.summary) report.summary.version = 'v0.73.0';
fs.writeFileSync('audit-v73.json', JSON.stringify(report, null, 2));

assert.equal(report.oposiciones.length, 4);
assert.equal(report.totalQuestions, 2912, `Total inesperado: ${report.totalQuestions}`);
...
assert.equal(uc3m.questions, 500);
console.log('Auditoría de calidad v0.73 OK', JSON.stringify(report.summary));
