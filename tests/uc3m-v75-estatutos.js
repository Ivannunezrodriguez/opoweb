...
assert.ok(ope && release, 'No se cargó UC3M v0.75');
fs.writeFileSync('uc3m-v75.json', JSON.stringify(release, null, 2));

assert.equal(release.totalQuestions, 560);
assert.equal(release.targetThemes, 18);
...
assert.equal(bank.length, 30, `Tema 15: ${bank.length}`);
assert.equal(new Set(bank.map(question => question.text)).size, 30, 'Preguntas duplicadas en tema 15');
assert.equal(new Set(bank.map(question => question.id)).size, 30, 'IDs duplicados en tema 15');
...
assert.ok(bank.every(question => question.justification && /uc3m\.es/.test(question.source)));
...
assert.ok(bank.some(question => /mitad más uno/i.test(question.options.map(option => option.text).join(' '))));
assert.ok(bank.some(question => /tres años/i.test(question.options.map(option => option.text).join(' '))));
assert.ok(bank.some(question => /anual, único, público y equilibrado/i.test(question.options.map(option => option.text).join(' '))));
assert.ok(bank.some(question => /tres meses anteriores/i.test(question.options.map(option => option.text).join(' '))));
...
console.log(`UC3M v0.75 OK · ${release.totalQuestions} preguntas · ${release.targetThemes}/20 temas a 30 · mínimo ${release.minimumQuestions}`);
