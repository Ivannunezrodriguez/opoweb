...
assert.ok(/const CACHE = 'opoweb-v81';/.test(serviceWorker), 'La caché no es opoweb-v81');
...
assert.ok(index.includes('assets/js/uc3m-v74-losu.js'), 'Falta cargar UC3M v0.74');
assert.ok(index.includes('assets/js/auditoria-calidad-v72.js'), 'Falta cargar auditoría de calidad');
assert.ok(index.includes('assets/js/ui-v74.js'), 'Falta cargar interfaz v0.74');

console.log(`Integridad web v0.74 OK · ${indexAssets.length} referencias · ${cachedAssets.length} recursos PWA`);
