...
report.version = 'v0.75.0';
report.date = '2026-07-12';
if (report.summary) report.summary.version = 'v0.75.0';
fs.writeFileSync('audit-v75.json', JSON.stringify(report, null, 2));

assert.equal(report.oposiciones.length, 4);
assert.equal(report.totalQuestions, 2972, `Total inesperado: ${report.totalQuestions}`);
...
const uc3m = report.oposiciones.find(item => item.id === 'uc3m-aux-admin-2026');
assert.equal(uc3m.questions, 560);
console.log('Auditoría de calidad v0.75 OK', JSON.stringify(report.summary));
