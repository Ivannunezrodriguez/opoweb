...
vm.runInNewContext(fs.readFileSync('assets/js/puebla-v67.js', 'utf8'), context);

const puebla = window.OPOSICIONES_DATA.oposiciones[0];
assert.equal(window.OPOWEB_PUEBLA_V67.totalQuestions, 570);
assert.equal(window.OPOWEB_PUEBLA_V67.targetThemes, 19);
assert.equal(window.OPOWEB_PUEBLA_V67.minimumQuestions, 30);
assert.equal(window.OPOWEB_PUEBLA_V67.practicalCases, 20);
...
console.log('Smoke La Puebla v0.67 OK');
