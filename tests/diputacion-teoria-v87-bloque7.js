...
const expectedCompleted = Array.from({ length: 14 }, (_, index) => index + 25);
const expectedPending = [39, 40];
...
const allowedReferences = new Set([
  'LO-BG24-MAIN', 'LO-BASE-HELP', 'LO-BASE-GUIDE', 'LO-ODB-FORMAT',
  'LO-IG24-MAIN', 'LO-IMPRESS-HELP', 'LO-IMPRESS-GUIDE', 'LO-ODP-FORMAT'
]);
const failures = [];
const metrics = [37, 38].map(number => {
  ...
  const blockQuestions = questions.filter(question => question.id.startsWith(`dip-v87-t${number}-b7-`));
  const metric = {
    number,
    title: theme.title,
...
const theme37 = ope.themes.find(item => Number(item.number) === 37);
const theme38 = ope.themes.find(item => Number(item.number) === 38);
if (!/LibreOffice 24 III/.test(theme37?.title || '') || !/Base/.test(theme37?.title || '')) failures.push('Numeración: tema 37 incorrecto');
if (!/LibreOffice 24 IV/.test(theme38?.title || '') || !/Impress/.test(theme38?.title || '')) failures.push('Numeración: tema 38 incorrecto');
...
if (programme?.autonomousThemes !== 14) failures.push(`Autónomos: ${programme?.autonomousThemes}`);
...
if (state?.block !== 7) failures.push('Bloque global incorrecto');
if (state?.numberingCheck?.baseTheme !== 37 || state?.numberingCheck?.impressTheme !== 38) failures.push('Control de numeración incorrecto');

const report = {
  version: 'v0.87-editorial-b7',
...
assert.deepStrictEqual(failures, [], `Diputación bloque 7 no supera la auditoría: ${failures.join(' | ')}`);
console.log(`Diputación teoría v87 bloque 7 OK · temas 37 y 38 · mínimo ${report.minimumWords} palabras`);
