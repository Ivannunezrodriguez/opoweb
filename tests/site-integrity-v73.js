...
assert.ok(/const CACHE = 'opoweb-v80';/.test(serviceWorker), 'La caché no es opoweb-v80');
...
assert.ok(index.includes('assets/js/uc3m-v73-transparencia.js'), 'Falta cargar UC3M v0.73');
assert.ok(index.includes('assets/js/auditoria-calidad-v72.js'), 'Falta cargar auditoría de calidad');
assert.ok(index.includes('assets/js/ui-v73.js'), 'Falta cargar interfaz v0.73');

console.log(`Integridad web v0.73 OK · ${indexAssets.length} referencias · ${cachedAssets.length} recursos PWA`);
