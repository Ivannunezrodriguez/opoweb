(() => {
  ...
  const setRegistered = (id, applicationStatus) => {
    const ope = data.oposiciones.find(item => item.id === id);
    if (!ope) return null;
    ope.applicationStatus = { registered: true, ...applicationStatus };
    return ope;
  };

  setRegistered('puebla-aux-admin-2026', {
    registeredAt: '28/05/2026',
    evidence: 'Inscripción confirmada; justificantes conservados fuera del repositorio público.'
  });

  setRegistered('carranque-aux-admin-2026', {
    registeredAt: 'Confirmado por el usuario',
    evidence: 'Inscripción confirmada; justificantes conservados fuera del repositorio público.'
  });

  const diputacion = setRegistered('diputacion-toledo-admin-2026', {
  ...
  const uc3m = setRegistered('uc3m-aux-admin-2026', {
  ...
  if (uc3m) {
    ...
    const calendar = Array.isArray(uc3m.processCalendar)
      ? uc3m.processCalendar.filter(item => item.title !== registrationEntry.title)
      : [];
  ...
  window.OPOWEB_APPLICATION_STATUS_V87 = {
    updatedAt: '15/07/2026',
    registeredOpeIds: data.oposiciones.filter(item => item.applicationStatus?.registered).map(item => item.id),
    publicRepositoryContainsPersonalEvidence: false
  };
})();
