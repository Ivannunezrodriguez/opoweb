(() => {
  ...
  const curated = {
    8: {
      ...
      rows: [
        ...
        ['¿Qué exige una medida de policía que limita derechos?', 'Motivación y proporcionalidad.', ['Ausencia de expediente.', 'Secreto permanente.', 'Aprobación por una empresa privada.'], 'Artículos 35 de la Ley 39/2015 y 84.2 de la Ley 7/1985.'],
  ...
  const buildCurated = (themeNumber, row, index) => {
    ...
    return {
      id: `carranque-v68-t${themeNumber}-q${index + 1}`,
      text,
  ...
  carranque.themes.forEach(theme => {
    ...
    theme.auditStatus = (carranque.themeTests[theme.id] || []).length >= TARGET ? 'segunda vuelta mínima completada v0.68' : 'refuerzo pendiente';
  ...
  window.OPOWEB_CARRANQUE_V68 = { version: `v${VERSION}`, targetThemes, totalReal, completedThemes: coverage.filter(item => item.count >= TARGET).map(item => item.number), practicalCases: carranque.practicalCases.length };
})();
